...
<head>
    <title>Code coverage report for tests/utils/solid-auth-cli/solid-rest/src/rest-patch.js</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
    <div class='pad1'>
        <h1><a href="../../../../../index.html">All files</a> / <a href="index.html">tests/utils/solid-auth-cli/solid-rest/src</a> rest-patch.js</h1>
        <div class='clearfix'>
...
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-yes">8x</span>
<span class="cline-any cline-neutral">&nbsp;</span></td><td class="text"><pre class="prettyprint lang-js">let $rdf;
...
&nbsp;
  async patchContent (content, contentType, options) {
    ...
    <span class="missing-if-branch" title="if path not taken" >I</span>if (!parsePatch) {
<span class="cstat-no" title="statement not covered" >      return [415, `Unsupported patch content type: ${patch.contentType}`] </span>
    }
...
<span class="cstat-no" title="statement not covered" >      throw new Error(`500 : Patch: Target ${resource.contentType} file syntax error: ${err}`)</span>
    }
    return graph
  }
...
<span class="fstat-no" title="function not covered" >  ha</span>sh (text) {
<span class="cstat-no" title="statement not covered" >    return crypto.createHash('md5').update(text).digest('hex')</span>
  }
}
&nbsp;
...
<span class="cstat-no" title="statement not covered" >    throw new Error(`400: Patch document syntax error: ${err}`)</span>
  }
&nbsp;
...
<span class="cstat-no" title="statement not covered" >    throw new Error(`400 : No patch for ${targetURI} found. ${err}`)</span>
  }
&nbsp;
  ...
  <span class="missing-if-branch" title="if path not taken" >I</span>if (!insert &amp;&amp; !deleted) {
<span class="cstat-no" title="statement not covered" >    throw error(400, 'Patch should at least contain inserts or deletes.')</span>
  }
