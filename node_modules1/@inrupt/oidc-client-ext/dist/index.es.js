...
async function decodeJwt(token, key, options) {
    if (key) {
        ...
        return JWT.verify(token, convertedKey, options);
    }
    return JWT.decode(token);
}
async function privateJwkToPublicJwk(key) {
    return (await JWK.asKey(key, "public"));
}
...
export { clearOidcPersistentStorage, createDpopHeader, decodeJwt, generateJwkForDpop, generateJwkRsa, getBearerToken, getDpopToken, privateJwkToPublicJwk, registerClient, removeOidcQueryParam, signJwt, validateIdToken };
