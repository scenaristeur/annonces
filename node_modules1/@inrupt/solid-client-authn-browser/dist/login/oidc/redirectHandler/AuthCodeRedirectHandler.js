...
async function verifyIdToken(issuerConfig, client, issuer, idToken) {
    const jwks = await IssuerConfigFetcher_1.getJwks(issuerConfig);
    return oidc_client_ext_1.validateIdToken(idToken, jwks, issuer, client.clientId);
}
...
let AuthCodeRedirectHandler = class AuthCodeRedirectHandler {
    ...
    async handle(redirectUrl) {
        ...
        if (!(await verifyIdToken(issuerConfig, client, issuer, tokens.idToken))) {
            throw new Error(`Invalid ID token [${tokens.idToken}]. Possible issues are bad signature, or mismatching audience (expected [${client.clientId}])`);
        }
